...
const emptyStats = {
  ...
  end_date: undefined,
  categories: [],
  best_day: { date: undefined, text: undefined },
  human_readable_daily_average: undefined,
  human_readable_total: undefined,
  languages: [],
  editors: [],
};
...
export const getReadStats = async (): Promise<{
  ...
  try {
    if (!API_KEY) {
      return { status: 200, data: emptyStats };
    }
    ...
    if (status >= 400) return { status, data: emptyStats };
...
export const getALLTimeSinceToday = async (): Promise<{
  ...
  try {
    if (!API_KEY) {
      return { status: 200, data: { text: undefined, total_seconds: 0 } };
    }
